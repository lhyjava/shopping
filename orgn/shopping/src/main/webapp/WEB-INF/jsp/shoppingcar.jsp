	...
	$.ajax({
		...
		success:function(data){
			if(data == "success"){
				...
				var a = parseInt(zongjia);
				for (var i = 0; i < arr.length; i++) {
					a += parseInt($(arr[i]).html());
				}
				$("#total_price").html(a);
			}
		}
      ...
      <div class="table">
        <table width="100%" cellpadding="0" cellspacing="0" border="0">
          ...
          <c:forEach items="${requestScope.shoppingcarlist }" var="tmp">
            ...
            <td align="center"> <a href="javascript:void();" onclick="delcurrtr(this,${tmp.id });" class="cart_common">删除</a></td>
          </tr>
          </c:forEach>
        </tbody></table>
      </div>
