...
public class ShoppingController {
	...
	public String findgoods(Integer store_price_begin, Integer store_price_end, String goods_name,
	...
	@RequestMapping("/goodsedit.htm")
	public String goodsedit(Integer id, HttpServletRequest req) {
		List<Goodsclassplus> classlist = goodsclassservice.GoodsClassFindAllByLevel(0);
		req.setAttribute("classlistforgoods", classlist);
		List<Specplus> speclist = specservice.findall();
		req.setAttribute("specpluslistforgoods", speclist);
		List<Goodsbrand> brandlist = goodsbrandservice.findall();
		req.setAttribute("brandlistforgoods", brandlist);
		return "goods_edit";
	}

	// 编辑页提交
	@RequestMapping("/goodseditsubmit.htm")
	public String goodseditsubmit(Goods g) {
		int ret = goodsService.updateByPrimaryKeySelective(g);
