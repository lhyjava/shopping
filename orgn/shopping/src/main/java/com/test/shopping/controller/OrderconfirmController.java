...
public class OrderconfirmController {
	...
	public String orderconfirm(String[] ids,String userid, HttpServletRequest req) {
		...
		req.getSession().setAttribute("idslist", idslist);
	...
	public String ordersubmit(String[] ids,Integer userid,String[] img,String[] name,String[] specmessage,
		...
		//在购物车表中删除已购买的商品
		String idss = Arrays.toString(ids);
		idss =idss.replace("[", "").replace("]", "").substring(0, idss.length() - 2);
		shoppingcarService.deleteByid(idss);
		
		return "redirect:/findorder.htm?userid="+userid;
	}
