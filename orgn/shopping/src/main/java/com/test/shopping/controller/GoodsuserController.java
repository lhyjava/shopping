...
public class GoodsuserController {
	...
	public String selectByuserfindall(String username, String password, String power,String yzm,String yanzhengma, HttpServletRequest req,
			HttpSession session) {	
		List<User> userlist = userservice.selectByuserfindall();
		req.setAttribute("userlist", userlist);
		for (User user : userlist) {
			if (username.equals(user.getUsername()) && password.equals(user.getPassword()) && yzm.equals(yanzhengma)) {				
				// 登陆成功
	...
	//bug待解决
	/*
	 * @RequestMapping("/logout.htm") public String logoutshow(HttpServletRequest
	 * req) { req.getSession().removeAttribute("currentusername"); return "login"; }
	 */

	// add by lys 1205 end

	@RequestMapping("/zhuce.htm")
	public String zhuce() {
		
		return"zhuce";
	}
	
	@RequestMapping("/userzhuce.htm")
	public String userzhuce(User user,String repassword,HttpServletRequest req) {
	
		String n = userservice.selectByusername(user.getUsername());
		if(n==null) {		
